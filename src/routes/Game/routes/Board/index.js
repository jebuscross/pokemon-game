...
const BoardPage = () => {
  ...
  const fetchPlayer2 = useCallback(async () => {
    const player2Responce = await fetch(
      "https://reactmarathon-api.netlify.app/api/create-player"
    );
    const player2Request = await player2Responce.json();

    pokemonsContext.addPlayer2Pokemon(player2Request.data);
    ...
    if (choiceCard) {
      ...
      if (choiceCard.player === 1) {
        setPlayer1((prevState) =>
          prevState.filter((el) => el.id !== choiceCard.id)
        );
      }
